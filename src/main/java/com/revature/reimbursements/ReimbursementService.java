...
public class ReimbursementService {
    ...
    public List<ReimbursementResponse> getAllReimbursements() {
        ...
        return reimbursementDAO.getAllReimbursements().stream().map(ReimbursementResponse::new)
    ...
    public List<ReimbursementResponse> getReimbursementsByStatus(String status) {
        ...
        return reimbursementDAO.findReimbursementbyStatus(status).stream().map(ReimbursementResponse::new)
                .collect(Collectors.toList());
    }
    ...
    public List<ReimbursementResponse> getReimbursementsByType(String type) {
        ...
        return reimbursementDAO.findReimbursementbyType(type).stream().map(ReimbursementResponse::new)
                .collect(Collectors.toList());
    }
    ...
    public ResponseString approveOrDeny(ApproveOrDenyBody approveOrDenyBody, String resolverId) {
        ...
        String updateSuccessfullMessage = reimbursementDAO.approveOrDenyReimbursement(statusId,
                approveOrDenyBody.getReimbursementId(), resolverId);
    ...
    public ResponseString updateAmount(UpdateOwnReimbBody updateOwnReimbBody) {
        ...
        if (updateOwnReimbBody.getUpdateTo() == null || updateOwnReimbBody.getUpdateTo().length() <= 0 ||
                updateOwnReimbBody.getReimbursementId() == null
                || updateOwnReimbBody.getReimbursementId().length() <= 0) {
        ...
        if (Double.parseDouble(updateOwnReimbBody.getUpdateTo()) > 9999.99
                || Double.parseDouble(updateOwnReimbBody.getUpdateTo()) <= 0) {
            throw new InvalidRequestException("Provided amount must be between 0.01 and 9999.99");
        }
    ...
    public ResponseString updateTypeId(UpdateOwnReimbBody updateOwnReimbBody) {
        ...
        if (!updateOwnReimbBody.getUpdateTo().equals("lodging") && !updateOwnReimbBody.getUpdateTo().equals("travel")
                && !updateOwnReimbBody.getUpdateTo().equals("food")
                && !updateOwnReimbBody.getUpdateTo().equals("other")) {
